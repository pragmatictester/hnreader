...
var hnreader_easytoread = {
   ...
   **/
	getSiteFavicon : function (hnBrowser, webPage, target) {
		...
		doc.documentElement.innerHTML = webPage;
		
		var links = doc.getElementsByTagName("link");
		
		wHref = target.protocol + "//" + target.hostname + "/favicon.ico";
		for (i = 0; i < links.length; i++) {
			...
			if (s.indexOf("icon") != -1) {
				...
				wHref = target.protocol + "//" + target.host + "/favicon.ico";
			};
		};
